...
#[test]
fn test_improve_variable_names_renames_cached_key_lookup_helper() {
                        ...
                        body: PBox::new(PseudoExpr::If {
                            ...
                            else_branch: PBox::new(PseudoExpr::If {
                                ...
                                else_branch: PBox::new(PseudoExpr::Apply {
                                    ...
                                    args: vec![PseudoExpr::var("tail"), PseudoExpr::var("y_2")]
                                        .into(),
                                }),
                            }),
                        }),
        ...
        PseudoExpr::RecFn { name, params, body } => {
            ...
            assert!(
                body_str.contains("name: \"needle\"")
                    ...
                    && body_str.contains("name: \"lookup\"")
                    && !body_str.contains("x_3")
                    && !body_str.contains("y_2")
                    && !body_str.contains("decode_credential_2"),
                "expected cached-key lookup helper to rename through body, got: {body_str}"
            );
        }
...
#[test]
fn test_improve_variable_names_renames_false_like_cached_key_lookup_helper() {
    let expr = PseudoExpr::RecFn {
        name: "decode_credential_2".to_string().into(),
        params: vec!["x_3".to_string().into(), "y_2".to_string().into()],
        body: PBox::new(PseudoExpr::When {
        ...
        PseudoExpr::RecFn { name, params, body } => {
            ...
            assert!(
                body_str.contains("name: \"needle\"")
                    ...
                    && !body_str.contains("y_2")
                    && !body_str.contains("decode_credential_2"),
                "expected false-like cached-key lookup helper to rename through body, got: {body_str}"
            );
        }
        other => panic!("expected recfn, got: {other:?}"),
    }
...
#[test]
fn test_improve_variable_names_renames_le_cutoff_cached_key_lookup_helper() {
    let expr = PseudoExpr::RecFn {
        name: "decode_credential_2".to_string().into(),
        params: vec!["x_3".to_string().into(), "y_2".to_string().into()],
        body: PBox::new(PseudoExpr::When {
        ...
        PseudoExpr::RecFn { name, params, body } => {
            ...
            assert!(
                body_str.contains("name: \"needle\"")
                    ...
                    && !body_str.contains("y_2")
                    && !body_str.contains("decode_credential_2"),
                "expected <= cutoff cached-key lookup helper to rename through body, got: {body_str}"
            );
        }
        other => panic!("expected recfn, got: {other:?}"),
    }
